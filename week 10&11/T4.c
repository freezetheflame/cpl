...
int main(){
  ...
  for(int i=1;i<=m;i++)
  {
    stringl=malloc(2000*sizeof(char));
    scanf("%s",stringl);
    int place=hash(stringl);
    if(a[place]==NULL)
      a[place]=createNODE(stringl);//该哈希值暂未使用过，故创造新的根结点
  ...
  for(int i=1;i<=q;i++)
  {
    st=malloc(2000*sizeof(char));
    scanf("%s",st);
    int ans=hash(st);
    if(a[ans]==NULL)printf("No\n");
    else {	if(strcmp(a[ans]->s,st)==0)printf("Yes\n");
